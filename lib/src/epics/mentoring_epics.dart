...
class MentoringEpics {
  ...
  Stream<AppAction> _addMentorship(
    ...
    return actions //
        .flatMap((AddMentorship$ action) => Stream<AddMentorship$>.value(action)
            .asyncMap((AddMentorship$ action) => _api.addMentorship(
                store.state.mentorships.info, store.state.auth.user))
            .map(
                (Mentorship mentorship) => AddMentorship.successful(mentorship))
            .onErrorReturnWith((dynamic error) => AddMentorship.error(error)));
  }
  ...
  Stream<AppAction> _updateMentorship(
    ...
    return actions //
        .flatMap((UpdateMentorship$ action) =>
            Stream<UpdateMentorship$>.value(action)
                .asyncMap((UpdateMentorship$ action) => _api.updateMentorship(
                    store.state.mentorships.info, action.id))
                .map((Mentorship mentorship) =>
                    UpdateMentorship.successful(mentorship))
                .onErrorReturnWith(
  ...
  Stream<AppAction> _deleteMentorship(
    ...
    return actions //
        .flatMap((DeleteMentorship$ action) =>
            Stream<DeleteMentorship$>.value(action)
                .asyncMap((DeleteMentorship$ action) =>
                    _api.deleteMentorship(action.id))
                .mapTo(const DeleteMentorship.successful())
                .onErrorReturnWith(
  ...
  Stream<AppAction> _getAllMentorships(
    ...
    return actions //
        .flatMap((GetAllMentorships$ action) =>
            Stream<GetAllMentorships$>.value(action)
                .asyncMap(
                    (GetAllMentorships$ action) => _api.getAllMentorships())
                .expand((List<Mentorship> mentorships) => <AppAction>[
