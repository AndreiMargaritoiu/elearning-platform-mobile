...
class TrackingEpics {
  ...
  Stream<AppAction> _trackAction(
    ...
    return actions //
        .flatMap((TrackAction$ action) => Stream<TrackAction$>.value(action)
            .asyncMap((TrackAction$ action) => _api.trackAction(
                store.state.trackedItems.info, store.state.auth.user.uid))
            .mapTo(const TrackAction.successful())
            .onErrorReturnWith((dynamic error) => TrackAction.error(error)));
  }
}
