...
class PostsEpics {
  ...
  Stream<AppAction> _createPost(
    ...
    return actions //
        .flatMap(
      (CreatePost$ action) => Stream<CreatePost$>.value(action)
          .asyncMap(
            (CreatePost$ action) => _api.createPost(
                store.state.posts.info, store.state.auth.user.uid),
          )
          .map(
            (Post post) => CreatePost.successful(post),
          )
          .onErrorReturnWith(
            (dynamic error) => CreatePost.error(error),
          ),
    );
  }

  Stream<AppAction> _listenForPosts(
      Stream<ListenForPosts$> actions, EpicStore<AppState> store) {
    return actions //
        .flatMap(
      (ListenForPosts$ action) => Stream<ListenForPosts$>.value(action)
          .asyncMap(
            (ListenForPosts$ action) => _api.listenForPosts(
              <String>[
